...
def generate_data(n):
    ...
    enemies = generate_enemies(n)

    return (battlemap, enemies)

def generate_enemies(n):
    enemymap = np.array([np.random.permutation([1 * np.random.uniform(low=0.4, high=1)] * math.ceil(1) + [0] * math.floor(n - 1)) for i in range(math.ceil(n / 2))] + [[0] * n for i in range(math.floor(n / 2))])
...
class BattlefieldAI:
    ...
    def reset(self):
        self.friendly_unit = Unit(100, 10, Pos(10, 10))
        self.frame_iteration = 0
        self.score = 0
        self.enemies = generate_enemies(n)
    ...
    def play_step(self, action):
        ...
        self.friendly_unit.health -= pos_decay
        
        # 3. check if game over
        reward = 0
        game_over = False
        if self.friendly_unit.health < 0 or self.frame_iteration > 1000:
            game_over = True
            reward = -1
            return reward, game_over, self.score
